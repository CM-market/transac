...
pub trait MediaStorage {
    async fn upload_media(&self, product_id: Uuid, multipart: &mut Multipart) -> Result<String, String>;
...
impl S3MediaStorage {
    ...
    async fn extract_file_from_multipart(&self, multipart: &mut Multipart) -> Result<(String, Bytes), String> {
...
impl MediaStorage for S3MediaStorage {
    async fn upload_media(&self, product_id: Uuid, multipart: &mut Multipart) -> Result<String, String> {
        ...
        let s3_key = format!("products/{}/media_{}.{}", product_id, Uuid::new_v4(), file_extension);
        
        // Upload to S3/MinIO
        let _result: PutObjectOutput = self.client
            .put_object()
    ...
    async fn delete_media(&self, media_key: &str) -> Result<(), String> {
        let _result: DeleteObjectOutput = self.client
            .delete_object()
            .bucket(&self.bucket_name)
            .key(media_key)
            .send()
...
impl MediaStorage for StubMediaStorage {
    async fn upload_media(&self, product_id: Uuid, _multipart: &mut Multipart) -> Result<String, String> {
        // Return a stubbed S3 key for development
        Ok(format!("products/{}/media_stub_{}.jpg", product_id, Uuid::new_v4()))
    }
